...
const overlaps=(startA,durA,startB,durB,buffer=0)=>{const A1=startA.getTime()-buffer*60000; const A2=A1+(durA+buffer*2)*60000; const B1=startB.getTime()-buffer*60000; const B2=B1+(durB+buffer*2)*60000; return A1<B2 && B1<A2}
...
export default function App(){
  ...
  function nextRecurrenceDate(yyyyMMdd,rule){
    ...
    switch(rule){case 'daily': d.setDate(d.getDate()+1); break; case 'weekdays': {const day=d.getDay(); d.setDate(d.getDate()+(day===5?3:1)); break} case 'weekly': d.setDate(d.getDate()+7); break; default: d.setDate(d.getDate()+1)}
    return toKey(d)
  }
  ...
  const doneToday=useMemo(()=>{ const today=toKey(new Date()); return tasks.filter(t=>t.done && t.scheduledStart && toKey(new Date(t.scheduledStart))===today).length }, [tasks])
  ...
  return (
    <div className="p-6 max-w-7xl mx-auto">
      <header className="mb-6 flex items-center gap-3">
        ...
        <span title="bomb" className="inline-flex items-center justify-center w-6 h-6 text-xs rounded-full" style={{background:'conic-gradient(from 45deg, #ef4444, #f59e0b, #10b981, #3b82f6, #a855f7, #ef4444)'}}>💣</span>
      </header>
        ...
        <div className="lg:col-span-2">
          <section className="border rounded-2xl p-4 mb-3 flex items-center justify-between">
            ...
            <div className="flex items-center gap-2">
              ...
              <button className={`px-3 py-2 rounded-lg ${selectedTaskId?'bg-emerald-600 text-white hover:bg-emerald-700':'bg-gray-100 text-gray-600'}`} onClick={()=> selectedTaskId && autoSchedule(true)}>Schedule Selected</button>
            </div>
          </section>
